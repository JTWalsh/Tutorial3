...
float imgBotLeftx = 0;
float imgBotLefty = -1;
float imgTopRightx = 1;
float imgTopRighty = 0;

double imageWidth = (abs(imgBotLeftx - imgTopRightx));
double imageHeight = (abs(imgBotLefty - imgTopRighty));
...
OpenGLImage::OpenGLImage(char *filename, GLuint id) {
    tx_ = 0;
    ty_ = 0;
    theta_ = 0;
    sx_ = sy_ = 1;
...
double xWorldToTexture(float Xvertex){
	return (abs(imgBotLeftx - Xvertex)) / imageWidth;
}

double yWorldToTexture(float Yvertex){
	return (abs(imgBotLefty - Yvertex)) / imageHeight;
}
...
void OpenGLImage::draw() {
	...
	//if (textureID_ == 1){
		glBegin(GL_QUADS); {
			glTexCoord2d(xWorldToTexture(startxBotLeft), yWorldToTexture(startyBotLeft));
			glVertex2d(-1, 0);

			glTexCoord2d(xWorldToTexture(startxTopRight), yWorldToTexture(startyBotLeft));
			glVertex2d(0, 0);

			glTexCoord2d(xWorldToTexture(startxTopRight), yWorldToTexture(startyTopRight));
			glVertex2d(0, 1);

			glTexCoord2d(xWorldToTexture(startxBotLeft), yWorldToTexture(startyTopRight));
			glVertex2d(-1, 1);
		}
		glEnd();
	//}
